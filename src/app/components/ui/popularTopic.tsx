...
export default function PopularTopic({order, topic=" ", description=" ", className}: Props) {
  const base= "flex items-center gap-4 p-2 border-[#e5e7eb]"
  return (
    <div className={ `${base} ${order===5? "border-0":"border-b"} ${className}`}>
      <div className="bg-[#8397c4] text-white flex justify-center items-center rounded-full w-[30px] h-[30px]">{order}</div>
      <div>
